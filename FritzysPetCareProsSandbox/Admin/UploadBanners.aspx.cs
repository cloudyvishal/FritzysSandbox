...
{
    public partial class UploadBanners : System.Web.UI.Page
    {
        int PageId;
        ...
        protected void BindData()
        {
            try
            {
                ...
                if (myDt.Rows.Count > 0)
                {
                    ...
                    CommonFunctions.Setserial(GrdBaner, "srno");
                ...
                else
                {
                    ...
                    if (ImageName2 != "")
                    {
                            ...
                            && ImageNameMobile != "" && ext == ".gif" || ext == ".jpg" || ext == ".png" || ext == ".GIF" || ext == ".jpeg" || ext == ".JPEG" || ext == ".PNG" || ext == ".JPG")
                        {
                            ...
                            string virtualpath2 =  "~/StoreData/BannerNew/" + ImageName2;

                            string virtualpathmobile = "~/StoreData/MobileBannerNew/" + ImageNameMobile;
        ...
        public void AddBannerToXml(string tempPath, string ImageName2, string ImageCoupon, string PageName, string PageId, string UserId, string virtualpathmobile, string BannerId)
        {
            try
            {
                ...
                xmldoc.Save(ContentManager.GetPhysicalPath(ConfigurationManager.AppSettings["HomePathValue"].ToString() + BannerOrder.BannerXml));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void DeleteFromXml(string PageName, int PageId, int UserId)
        {
            try
            {
                ...
                xmldoc.Save(ContentManager.GetPhysicalPath(ConfigurationManager.AppSettings["HomePathValue"].ToString() + "banners_Cat1.xml"));
            }
            catch (Exception ex)
            {
            }
