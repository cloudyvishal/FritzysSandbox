...
namespace FritzysPetCareProsSandbox.Admin
{
    public partial class ContactUs : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    CommonFunctions.Setserial(gdvContactUs, "srno");
                ...
                else
                {
                    if ((Convert.ToInt32(ddlSearch.SelectedIndex) > 0) && (txtSearch.Text != ""))
                    {
                        ...
                        ErrMessage("Sorry no recordes avalible");
                    }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                string ContactID = CommonFunctions.GetCheckedRow(gdvContactUs, "chkSelect");
                if (ContactID != "")
                {
                    StoreFront ObjStoreFront = new StoreFront();
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DateTime dtFirst = DateTime.Parse(txtStartDate.Text.Trim());

                DateTime dtSecond = DateTime.Parse(txtEndDate.Text.Trim());

                ds = ObjStoreFront.GetAllContactUsExport(dtFirst, dtSecond);
