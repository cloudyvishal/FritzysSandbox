...
{
    public partial class DogAndPetFritzyFreshSpa : BasePage
    {
        int ServiceId = 43;
        int pageid = 1;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (SessionManagement.UserId.ToString() == "3")
                    Count = 3;
                if (SessionManagement.UserId.ToString() == "2")
                    Count = 2;
                if (SessionManagement.UserId.ToString() == "1")
                    Count = 1;
                if (SessionManagement.IsLogin.ToString() == "" || SessionManagement.IsLogin.ToString() == null || SessionManagement.IsLogin.ToString() == "0")
                {
                    Count = 0;
                }
                    ...
                    new[]
                {
                    ...
                    {
                        Response.Redirect("http://localhost:50372/fritzymobile/Dog_and_Pet_FritzyFreshSpa.aspx".ToLower());
                        HttpContext.Current.Response.Status = "301 Moved Permanently";
                        HttpContext.Current.Response.AddHeader("Location", Request.Url.ToString().Replace("http://localhost:50372/", "http://www.localhost:50372/Dog_and_Pet_FritzyFreshSpa.aspx"));
        ...
        public void BindData(int ServiceID, int PageID)
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    if (PageID == 1)
                    {
                        ImgService.ImageUrl = SessionManagement.HomePath + "StoreData/Images/" + ds.Tables[0].Rows[0]["Image"].ToString();
                    ...
                    if (PageID == 2)
                    {
                        ImgService.ImageUrl = SessionManagement.HomePath + "StoreData/HomeServices/" + ds.Tables[0].Rows[0]["Image"].ToString();
                        lblServiceTitle.Text = ds.Tables[0].Rows[0]["ServiceTitle"].ToString();
                        Page.Title = ds.Tables[0].Rows[0]["ServiceTitle"].ToString();
                        lblServiceDesc.Text = ds.Tables[0].Rows[0]["ServiceDescription"].ToString();
                    }
                    if (PageID == 3)
                    {
                        ImgService.ImageUrl = SessionManagement.HomePath + "StoreData/ServicePageServices/" + ds.Tables[0].Rows[0]["Image"].ToString();
                        lblServiceTitle.Text = ds.Tables[0].Rows[0]["ServiceTitle"].ToString();
                        Page.Title = ds.Tables[0].Rows[0]["ServiceTitle"].ToString();
                        lblServiceDesc.Text = ds.Tables[0].Rows[0]["ServiceDescription"].ToString();
                    }
